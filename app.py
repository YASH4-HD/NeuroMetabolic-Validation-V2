...
if not df_kegg.empty:
    ...
    if uploaded_file:
        try:
            ...
            st.error("CSV Error: Please ensure columns for 'Symbol' and 'LogFC' exist.")
            df_kegg['LogFC'] = 0
    else:
        df_kegg['LogFC'] = 0
    ...
    with m_tab1:
        ...
        hub_str = " and ".join([", ".join(hub_names[:-1]), hub_names[-1]]) if len(hub_names) > 1 else "N/A"
        ...
        st.text_area("Suggested Caption:", cap, height=130)

    with m_tab2:
        meth = f"Systems biology analysis was performed using a custom pipeline. Pathway genes were retrieved from KEGG ({pathway_id}). Protein-protein interactions were mapped using the STRING-DB API with a confidence score of {confidence/1000}. Network visualization and degree centrality were calculated using NetworkX."
